...
export default function PortfolioAllocation({ holdings, portfolios, activePortfolioId }: Props) {
    ...
    if (portfolioValues.length === 0) {
      return (
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 backdrop-blur-sm">
          ...
          <p className="text-gray-500 text-center py-8">No holdings across portfolios yet.</p>
        </div>
      );
    }
    ...
    return (
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {portfolioValues.map((pv) => (
            <div key={pv.portfolio.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-gray-700/30">
              <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: pv.portfolio.color }} />
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-200 truncate">{pv.portfolio.name}</p>
                <p className="text-xs text-gray-500">{pv.count} holding{pv.count !== 1 ? "s" : ""}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-white font-medium">{fmt(pv.value)}</p>
                <p className="text-xs text-gray-400">{total > 0 ? ((pv.value / total) * 100).toFixed(1) : 0}%</p>
              </div>
            </div>
          ))}
        </div>
  ...
  if (tickerValues.length === 0) {
    return (
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 backdrop-blur-sm">
        <h2 className="text-lg font-semibold text-white mb-4">Holdings Allocation</h2>
        <p className="text-gray-500 text-center py-8">No priced holdings in this portfolio.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 backdrop-blur-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {tickerValues.map((t, i) => (
          <div key={t.ticker} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-gray-700/30">
            <span
              className="w-3 h-3 rounded-full flex-shrink-0"
              style={{ backgroundColor: tickerColors[i % tickerColors.length] }}
            />
            <div className="flex-1 min-w-0">
              <p className="text-sm text-gray-200 font-medium">{t.ticker}</p>
              <p className="text-xs text-gray-500 truncate">{t.name}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-white font-medium">{fmt(t.value)}</p>
              <p className="text-xs text-gray-400">{((t.value / total) * 100).toFixed(1)}%</p>
            </div>
          </div>
        ))}
      </div>
